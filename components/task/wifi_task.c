...
void wifi_task(void *pvParameters)
{
	...
	sprintf(wifi_info.wifi_mac, "%s", wifi_get_mac());
	wifi_init_sta();
	wait_for_ip();
...
static esp_err_t event_handler_wifi(void *ctx, system_event_t *event)
{
	...
	case SYSTEM_EVENT_STA_CONNECTED:
		...
		device_data.wifi_status = E_WIFI_CONNECTED;
	...
	case SYSTEM_EVENT_STA_DISCONNECTED:
		...
		device_data.wifi_status = E_WIFI_DISCONNECT;
...
static void wifi_init_sta(void)
{
	...
	strcpy((char *)xWifi_Config.sta.ssid, wifi_info.wifi_name);
	strcpy((char *)xWifi_Config.sta.password, wifi_info.wifi_pass);

	APP_LOGI("--- main: Station copy = %s, Password = %s", xWifi_Config.sta.ssid,
			 xWifi_Config.sta.password);
